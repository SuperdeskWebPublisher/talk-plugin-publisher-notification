...
const sendData = (asset, commentCount) => {
  process.nextTick(async () => {
    const response = await fetch(
      process.env.TALK_PUBLISHER_API_URL + "content/articles",
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          "x-superdesk-signature":
            "sha1=" + process.env.TALK_PUBLISHER_AUTH_TOKEN
        },
        body: JSON.stringify({
          url: asset.url,
    ...
    if (!response.ok) {
      ...
      console.trace(
        `Posting comment count to Publisher failed with HTTP code ${response.status} and body '${responseText}'`
      );
    }
  });
};
  ...
  RootMutation: {
    createComment: {
      async post(root, args, context, info, result) {
        ...
        const commentCount = await CommentsLoader().Comments.countByAssetID.load(
          result.comment.asset_id
        );
